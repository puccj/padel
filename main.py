...
def main ():
    ...
    out_video, fps, out_csvs = analyzer.process_all(method=PadelAnalyzer.Method.ACCURATE, debug=True)
    # analyzer.process_all(method=PadelAnalyzer.Method.FAST)

    for csv in out_csvs:
        csv_analyzer = CsvAnalyzer(out_csvs[0], fps)
        ...
        csv_analyzer.create_graphs()    # Sorpresa !
