...
bool Padel::process(int delay, std::string outputVideo, std::string outputData, int timeLimit, bgSubMode mode, bool removeShadows) {
  ...
  while (true) {
    ...
    */
    ...
    cv::circle(field, _lastPos[0]*zoom+offset, 15, {255, 255, 255}, cv::FILLED, cv::LINE_AA);
    cv::circle(field, _lastPos[1]*zoom+offset, 15, {0, 0, 0}, cv::FILLED, cv::LINE_AA);
    cv::circle(field, _lastPos[2]*zoom+offset, 15, {100, 100, 255}, cv::FILLED, cv::LINE_AA);
    cv::circle(field, _lastPos[3]*zoom+offset, 15, {255, 0, 0}, cv::FILLED, cv::LINE_AA);
